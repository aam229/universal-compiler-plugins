...
function entityItemsReducer(entityItemsState = {}, action, entityType) {
  ...
  return {
    ...
    ...itemsData.reduce((entities, entityData) => {
      if (entityData) {
        entities[entityData.id] = entityItemReducer(
          entityItemsState[entityData.id],
          action,
          entityData,
        );
      } else {
...
function create(type, id, isCollection, createPromise) {
  return (dispatch, getState) =>
    // dispatch(createRequestAction(type, null, false, CREATE_OPERATION));
     createPromise(dispatch, getState).then((result) => {
       dispatch(createReceiveAction(
         result,
         CREATE_OPERATION,
         isCollection ? type : null,
